...
void libvlc_set_video_filter( libvlc_media_player_t *p_mi, const char *psz_name, bool b_enable )
{
    ...
    get_filter_str( VLC_OBJECT(p_mi), psz_name, b_enable, &psz_filter_type, &psz_string);
...
void libvlc_send_mouse_event(libvlc_media_player_t *p_mi, int VoutID, unsigned type, int btn, int x, int y)
{
    ...
    if(btn >= 0) {
        if(type == 0) {
            ...
            vout_window_SendMouseEvent(((vout_thread_sys_t*)pp_vouts[VoutID]->p)->display_cfg.window, &mouse);
        } else if(type == 1) {
            // Released
            const vout_window_mouse_event_t mouse = {
