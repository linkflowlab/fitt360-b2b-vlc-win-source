...
# define MODULE_VARNAME "gl"
    set_shortname (N_("OpenGL_stitch"))
    set_description (N_("OpenGL video output with stitch"))
    set_category (CAT_VIDEO)
    set_subcategory (SUBCAT_VIDEO_VOUT)
    set_capability ("vout display", 265)
    set_callbacks (Open, Close)
    add_shortcut ("opengl_stitch", "gl_stitch")
                           ...
                           vlc_tick_t date)
{
    ...
    {
#if PROJECTION_TESTMODE == 1
		vout_display_opengl_SetProjection(sys->vgl, PROJECTION_MODE_RECTANGULAR);
#endif
